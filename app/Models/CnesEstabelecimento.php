...
class CnesEstabelecimento extends Model
{
    ...
    public function buscar(array $filtros = [], int $pagina = 1, int $porPagina = 50): array
    {
        ...
        if (!empty($filtros['uf'])) {
            // Aceitar tanto sigla (RJ) quanto código IBGE (33)
            // Mapeamento sigla -> código IBGE para busca
            static $siglaParaIbge = [
            ...
            $ufFiltro = strtoupper($filtros['uf']);
            $codigoIbge = $siglaParaIbge[$ufFiltro] ?? null;
            if ($codigoIbge) {
                // Buscar por sigla OU código IBGE (compatibilidade com diferentes versões do CSV)
                $where[]  = '(co_estado_gestor = ? OR co_estado_gestor = ?)';
                $params[] = $ufFiltro;
                $params[] = $codigoIbge;
            } else {
