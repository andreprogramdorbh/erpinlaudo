...
$statusConfig = [
    ...
    'entregue'   => ['label' => 'Entregue',    'bg' => '#d1fae5', 'color' => '#065f46', 'icon' => 'fa-truck'],
    'faturado'   => ['label' => 'Faturado',    'bg' => '#ede9fe', 'color' => '#5b21b6', 'icon' => 'fa-file-invoice'],
...
<div class="d-flex align-items-start gap-3 mb-4">
    <div style="width:52px;height:52px;border-radius:12px;background:#d1fae5;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
        <i class="fas fa-shopping-bag" style="color:#059669;font-size:22px;"></i>
    </div>
    <div class="flex-grow-1">
        <div class="d-flex align-items-center gap-3 flex-wrap">
            <h4 class="mb-0">Pedido de Venda <?= $esc($pedido->numero ?? '#' . $pedido->id) ?></h4>
    ...
    <div class="d-flex gap-2 flex-wrap">
        ...
        <a href="/estoque/vendas/<?= (int)$pedido->id ?>/edit" class="btn btn-outline-primary btn-sm">
            <i class="fas fa-edit me-1"></i> Editar
        </a>
        <?php endif; ?>
        <?php if ($st === 'confirmado'): ?>
        <form method="POST" action="/estoque/vendas/<?= (int)$pedido->id ?>/expedir" class="d-inline">
            <input type="hidden" name="csrf_token" value="<?= $esc($_SESSION['csrf_token'] ?? '') ?>">
            <button type="submit" class="btn btn-success btn-sm"
                onclick="return confirm('Confirmar expedição e baixar o estoque?')">
                <i class="fas fa-truck me-1"></i> Expedir Pedido
            </button>
        </form>
        <?php endif; ?>
        <?php if (in_array($st, ['rascunho', 'confirmado'])): ?>
        <form method="POST" action="/estoque/vendas/<?= (int)$pedido->id ?>/cancelar" class="d-inline">
            <input type="hidden" name="csrf_token" value="<?= $esc($_SESSION['csrf_token'] ?? '') ?>">
            <button type="submit" class="btn btn-outline-danger btn-sm"
                onclick="return confirm('Cancelar este pedido de venda?')">
        ...
        <div class="info-card">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <div class="info-value"><?= $esc($pedido->cliente_nome ?? '—') ?></div>
                </div>
                <div class="col-md-3">
                    <div class="info-label">CPF / CNPJ</div>
                    <div class="info-value"><?= $esc($pedido->cliente_cpf_cnpj ?? '—') ?></div>
                </div>
                <div class="col-md-3">
                    <div class="info-label">Telefone</div>
                    <div class="info-value"><?= $esc($pedido->cliente_telefone ?? '—') ?></div>
                </div>
                <div class="col-md-4">
                    <div class="info-label">E-mail</div>
                    <div class="info-value"><?= $esc($pedido->cliente_email ?? '—') ?></div>
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                    ...
                    <div class="info-value"><?= $esc($formas[$pedido->forma_pagamento ?? ''] ?? ($pedido->forma_pagamento ?? '—')) ?></div>
                </div>
                <?php if (!empty($pedido->endereco_entrega)): ?>
                <div class="col-12">
                    <div class="info-label">Endereço de Entrega</div>
                    <div class="info-value"><?= $esc($pedido->endereco_entrega) ?></div>
                </div>
                <?php endif; ?>
                ...
                <table class="table itens-table align-middle mb-0">
                    ...
                    <tbody>
                        <?php foreach ($pedido->itens as $item):
                            $tot   = (float)($item->valor_total ?? ($item->quantidade * $item->preco_unitario));
                            $custo = (float)($item->preco_custo ?? 0) * (float)($item->quantidade ?? 1);
                            $marg  = $tot - $custo;
                        ?>
                        <tr>
                            <td>
                            ...
                            <td class="text-end text-muted"><?= $fmt($item->preco_custo ?? 0) ?></td>
                            <td class="text-end fw-bold"><?= $fmt($tot) ?></td>
                            <td class="text-end <?= $marg >= 0 ? 'text-success' : 'text-danger' ?> fw-semibold">
                                <?= $fmt($marg) ?>
                            </td>
                        </tr>
                        <?php endforeach; ?>
    ...
    <div class="col-lg-4">
        <div class="info-card">
            ...
            <div class="d-flex justify-content-between border-top pt-3 mb-2">
                ...
                <strong style="font-size:20px;color:#059669;"><?= $fmt($pedido->valor_total ?? 0) ?></strong>
            </div>
            <div class="d-flex justify-content-between">
                <span class="text-muted small">Margem Bruta:</span>
                <?php $marg = (float)($pedido->margem_total ?? 0); ?>
                <span class="fw-semibold <?= $marg >= 0 ? 'text-success' : 'text-danger' ?>"><?= $fmt($marg) ?></span>
            </div>
            <?php if (!empty($pedido->valor_custo_total) && (float)$pedido->valor_total > 0): ?>
            <div class="d-flex justify-content-between mt-1">
                <span class="text-muted small">Margem %:</span>
                <?php $percMarg = ((float)$pedido->margem_total / (float)$pedido->valor_total) * 100; ?>
                <span class="fw-semibold <?= $percMarg >= 0 ? 'text-success' : 'text-danger' ?>">
                    <?= number_format($percMarg, 1, ',', '.') ?>%
                </span>
            </div>
            <?php endif; ?>
        </div>
